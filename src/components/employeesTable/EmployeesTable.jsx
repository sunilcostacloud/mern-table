...
const EmployeesTable = () => {
    ...
    const { data, isLoading, isError, error, isSuccess } = useSelector((state) => state.getEmployee);
    ...
    const handlePageChange = (event, newPage) => {
        ...
        dispatch(getEmployeeTable({ search, gender, status, sort, page: newPage }));
    ...
    const handleSearch = (searchedVal) => {
        ...
        setCurrentPage(1);
        if (searchedVal == "") {
            dispatch(getEmployeeTable({ search: searchedVal, gender, status, sort, page: 1 }));
        }
    }

    const requestSearch = () => {
        setCurrentPage(1);
        dispatch(getEmployeeTable({ search, gender, status, sort, page: 1 }));
    };

    const handleGenderChange = (event) => {
        setGender(event.target.value);
        setCurrentPage(1);
        dispatch(getEmployeeTable({ search, gender: event.target.value, status, sort, page: 1 }));
    };

    const handleStatusChange = (event) => {
        setStatus(event.target.value);
        setCurrentPage(1);
        dispatch(getEmployeeTable({ search, gender, status: event.target.value, sort, page: 1 }));
    };

    const handleSortChange = (event) => {
        setSort(event.target.value);
        setCurrentPage(1);
        dispatch(getEmployeeTable({ search, gender, status, sort: event.target.value, page: 1 }));
    };



    useEffect(() => {
        dispatch(getEmployeeTable({ search, gender, status, sort, page: currentPage }));
